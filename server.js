...
app.post('/upload-bulk', upload.array('files', 10), async (req, res) => {
    try {
        ...
        for (let i = 0; i < req.files.length; i++) {
            ...
            // Eagerly process with the selected background color
            const result = await cloudinary.uploader.upload(file.path, {
                resource_type: "auto",
                eager: [
                    { effect: "background_removal" },
                    { width: 1200, height: 1200, crop: "pad", background: bgColor, fetch_format: "jpg", quality: "auto" }
                ],
                eager_async: false 
            });
            ...
            results.push({ link, price, previewUrl: result.eager[0].secure_url });
        }
