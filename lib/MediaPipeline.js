...
const IMAGE_CARD_WIDTH = 1200;
const IMAGE_CARD_HEIGHT = 1800;
...
function buildImageTransformations({ shouldRemoveBg, bgColor, watermarkText, badgeLabel }) {
    ...
    transformations.push({ width: IMAGE_CARD_WIDTH, height: IMAGE_CARD_HEIGHT, crop: 'pad', background: safeBg });
    ...
    transformations.push({ quality: 'auto:best', fetch_format: 'auto', dpr: '2.0' });
...
function buildImagePreviewUrl(cloudinary, { publicId, bgColor = 'white', removeBg = false, badgeLabel = '' }) {
    ...
    transformations.push({ width: IMAGE_CARD_WIDTH, height: IMAGE_CARD_HEIGHT, crop: 'pad', background: bgColor });
    const safeBadge = normalizeBadgeLabel(badgeLabel);
